...
// import ContextMenu from "./drawing-old/ContextMenu";
// import CornerClipModal from "./drawing-old/CornerClipModal";
// import CornerRadiusModal from "./drawing-old/CornerRadiusModal";
// import CursorPanel from "./drawing-old/CursorPanel";
// import DistanceModal from "./drawing-old/DistanceModal";
// import InfoLog from "./drawing-old/InfoLog";
// import SidePanel from "./drawing-old/SidePanel";
// import URLImage from "./drawing-old/URLImage";
// import {
// 	DEFAULT_SIZE,
// 	EDGE_TOLERANCE,
// 	IMAGE_OPTIONS,
...
// 	STAGE_HEIGHT,
// 	STAGE_WIDTH,
// } from "./drawing-old/constants";
// import { useContextMenuController } from "./drawing-old/hooks/useContextMenuController";
// import { useSinkDrag } from "./drawing-old/hooks/useSinkDrag";
// import { useStageNavigation } from "./drawing-old/hooks/useStageNavigation";
// import { useTouchInteractions } from "./drawing-old/hooks/useTouchInteractions";
// import DragOverlayLayer from "./drawing-old/layers/DragOverlayLayer";
// import LabelsLayer from "./drawing-old/layers/LabelsLayer";
// import OutlineLayer from "./drawing-old/layers/OutlineLayer";
// import type {
// 	ContextMenuState,
// 	CornerName,
// 	DragDirection,
...
// 	RectShape,
// 	ToolMode,
// } from "./drawing-old/types";
// import { generateRandomRects } from "./drawing-old/utils/dev";
// import { draftForDrag as utilDraftForDrag } from "./drawing-old/utils/draft";
// import { hitTestBoundsEdge } from "./drawing-old/utils/edges";
// import { addRectPathWithCorners } from "./drawing-old/utils/geometry";
// import {
// 	collectGroups,
// 	getBounds,
// 	getGroupRectsForRect,
// } from "./drawing-old/utils/groups";
// import {
// 	findCornerAtPointer as htFindCorner,
// 	findEdgeAtPointer as htFindEdge,
// 	findRectAtPoint as htFindRect,
// } from "./drawing-old/utils/hitTest";
// import { exportJsonToImage } from "./drawing-old/utils/print";
...
// 		[],
// 	);
// 	const [draftMetas, setDraftMetas] = useState<
// 		ReadonlyArray<{ axis: "h" | "v"; origin: Point; end: Point; dir: 1 | -1 }>
// 	>([]);
...
// 	const [seamPreview, setSeamPreview] = useState<null | {
// 		groupKey: string;
// 		orientation: "v" | "h";
// 		at: number;
...
// 	const [defaultCornerColor, setDefaultCornerColor] =
...
// 	const [vainGroupTransforms, setVainGroupTransforms] = useState<
// 		Map<string, { x: number; y: number; scale: number; rotation: number }>
// 	>(new Map());
// 	const [vainActiveGroupKey, setVainActiveGroupKey] = useState<string | null>(
// 		null,
// 	);
...
// 				if (!next.has(key))
// 					next.set(key, { x: 0, y: 0, scale: 1, rotation: 0 });
// 			}
...
// 	const { menu, setMenu, closeMenu, openAtContainerPoint } =
...
// 					const inside =
// 						pt.x >= img.x &&
// 						pt.x <= img.x + img.width &&
// 						pt.y >= img.y &&
// 						pt.y <= img.y + img.height;
...
// 	const { sinkDrag, onSinkDragStart, onSinkDragMove, onSinkDragEnd } =
// 		useSinkDrag(rects, images, setImages, stageRef);
...
// 		): RectDraft =>
// 			utilDraftForDrag(origin, dx, dy, currentDirection, {
// 				SNAP_TO_ORIGIN,
// 				DEFAULT_SIZE,
// 				MIN_SIZE,
// 				SNAP_SIZE,
...
// 									splitMap.set(r.id, {
// 										...(splitMap.get(r.id) ?? { aId: leftRect.id, bId: "" }),
// 										aId: leftRect.id,
// 									});
// 								}
// 								if (rightWidth > 0) {
// 									const rightRect: RectShape = {
// 										...r,
// 										id: nextRectId(),
// 										x: at,
// 										width: rightWidth,
...
// 									splitMap.set(r.id, {
// 										...(splitMap.get(r.id) ?? { aId: "", bId: rightRect.id }),
// 										bId: rightRect.id,
// 									});
// 								}
// 							}
...
// 									result.push(topRect);
// 									splitMap.set(r.id, {
// 										...(splitMap.get(r.id) ?? { aId: topRect.id, bId: "" }),
// 										aId: topRect.id,
// 									});
// 								}
// 								if (bottomHeight > 0) {
// 									const bottomRect: RectShape = {
// 										...r,
// 										id: nextRectId(),
// 										y: at,
// 										height: bottomHeight,
...
// 									splitMap.set(r.id, {
// 										...(splitMap.get(r.id) ?? { aId: "", bId: bottomRect.id }),
// 										bId: bottomRect.id,
// 									});
// 								}
// 							}
...
// 					setImages((imgs) =>
// 						imgs.map((im) => {
// 							const pair = im.parentRectId
// 								? splitMap.get(im.parentRectId)
// 								: undefined;
// 							if (!pair) return im;
// 							if (orientation === "v") {
...
// 									...im,
// 									parentRectId:
// 										cx >= at ? pair.bId || pair.aId : pair.aId || pair.bId,
// 								};
// 							}
...
// 								...im,
// 								parentRectId:
// 									cy >= at ? pair.bId || pair.aId : pair.aId || pair.bId,
// 							};
// 						}),
// 					);
// 					return result;
...
// 						const inside =
// 							scenePoint.x >= it.x &&
// 							scenePoint.x <= it.x + it.width &&
// 							scenePoint.y >= it.y &&
// 							scenePoint.y <= it.y + it.height;
// 						if (inside) return it;
// 					}
// 					return null;
// 				})();
...
// 									if (
// 										Math.abs(scenePoint.x - gl) <= EDGE_TOLERANCE &&
// 										scenePoint.y >= gt - EDGE_TOLERANCE &&
// 										scenePoint.y <= gb + EDGE_TOLERANCE
// 									)
// 										gh = "left";
// 									else if (
// 										Math.abs(scenePoint.x - gr) <= EDGE_TOLERANCE &&
// 										scenePoint.y >= gt - EDGE_TOLERANCE &&
// 										scenePoint.y <= gb + EDGE_TOLERANCE
// 									)
// 										gh = "right";
// 									else if (
// 										Math.abs(scenePoint.y - gt) <= EDGE_TOLERANCE &&
// 										scenePoint.x >= gl - EDGE_TOLERANCE &&
// 										scenePoint.x <= gr + EDGE_TOLERANCE
// 									)
// 										gh = "top";
// 									else if (
// 										Math.abs(scenePoint.y - gb) <= EDGE_TOLERANCE &&
// 										scenePoint.x >= gl - EDGE_TOLERANCE &&
// 										scenePoint.x <= gr + EDGE_TOLERANCE
// 									)
// 										gh = "bottom";
// 									if (gh) {
// 										beginGroupResize(scenePoint, gRects, gh, {
// 											left: gl,
// 											top: gt,
// 											right: gr,
// 											bottom: gb,
...
// 								const group = rect.groupId
// 									? rects.filter((r) => r.groupId === rect.groupId)
// 									: [rect];
...
// 		if (
// 			(mode === "edge" || mode === "edge-new" || mode === "reshape") &&
...
// 			let grow = 0;
// 			if (edge === "right") {
// 				shrink = Math.max(0, -dx);
// 				grow = Math.max(0, dx);
// 			} else if (edge === "left") {
// 				shrink = Math.max(0, dx);
// 				grow = Math.max(0, -dx);
// 			} else if (edge === "bottom") {
// 				shrink = Math.max(0, -dy);
// 				grow = Math.max(0, dy);
// 			} else if (edge === "top") {
// 				shrink = Math.max(0, dy);
// 				grow = Math.max(0, -dy);
...
// 				if (edge === "right")
// 					return Math.abs(r.x + r.width - bounds.right) <= EDGE_TOLERANCE;
// 				if (edge === "left")
// 					return Math.abs(r.x - bounds.left) <= EDGE_TOLERANCE;
...
// 			let endIdx = -1;
// 			const bases = orderedIds
// 				.map((id) => rectBase.get(id))
// 				.filter((v): v is RectShape => !!v);
// 			if (edge === "right" || edge === "bottom") {
// 				for (let i = bases.length - 1; i >= 0; i -= 1) {
// 					if (touchAt(bases[i] as RectShape)) {
// 						endIdx = i;
// 						break;
// 					}
// 				}
...
// 							if (remaining >= DEFAULT_SIZE - 1e-6) {
// 								removedIds.add(seg.id);
// 								remaining = Math.max(remaining - DEFAULT_SIZE, 1);
// 								idx += step;
// 								continue;
// 							}
...
// 						if (base) {
// 							const endX =
// 								edge === "right" ? bounds.right : bounds.left - DEFAULT_SIZE;
// 							const centerY = base.y + (base.height - DEFAULT_SIZE) / 2;
// 							const newSeg: RectDraft = {
// 								x: endX,
// 								y: centerY,
// 								width: Math.max(DEFAULT_SIZE, grow),
// 								height: DEFAULT_SIZE,
// 							};
...
// 								setNewRect({
// 									x:
// 										endX + (edge === "right" ? newSeg.width - DEFAULT_SIZE : 0),
// 									y: vY,
// 									width: DEFAULT_SIZE,
// 									height: Math.max(MIN_SIZE, Math.abs(ortho)),
// 								});
// 							}
// 						}
// 					}
...
// 						if (base) {
// 							const endY =
// 								edge === "bottom" ? bounds.bottom : bounds.top - DEFAULT_SIZE;
// 							const centerX = (bounds.right + bounds.left) / 2; // approximate along group
// 							const newSeg: RectDraft = {
// 								x: centerX,
// 								y: endY,
// 								width: DEFAULT_SIZE,
// 								height: Math.max(DEFAULT_SIZE, grow),
// 							};
// 							setDraftSegments([newSeg]);
// 							const ortho = Math.abs(isHorizontalEdge ? dy : dx);
// 							if (grow >= L_TURN_MIN_PX && ortho > L_TURN_DEADBAND_PX) {
...
// 									y:
// 										endY +
// 										(edge === "bottom" ? newSeg.height - DEFAULT_SIZE : 0),
// 									width: Math.max(MIN_SIZE, Math.abs(ortho)),
// 									height: DEFAULT_SIZE,
// 								});
// 							}
// 						}
...
// 				setImages(
// 					baseImages.map((im) => ({ ...im, x: im.x + dx, y: im.y + dy })),
// 				);
// 			}
// 			return;
// 		}
...
// 		if (
// 			(mode === "edge" || mode === "edge-new") &&
// 			draggingRectId.current &&
// 			draggingRectOffset.current
// 		) {
// 			const pos = getScenePointerPosition();
// 			if (!pos) return;
...
// 			setRects((prev) =>
// 				prev.map((r) => (r.id === rectId ? { ...r, x: newX, y: newY } : r)),
// 			);
...
// 					(dy >= 0 ? 1 : -1) === (lastMeta.dir === 1 ? -1 : 1);
// 				if (
// 					verticalOpposite &&
// 					Math.abs(dy) > L_TURN_DEADBAND_PX &&
// 					Math.abs(dx) <= L_TURN_DEADBAND_PX
// 				) {
...
// 					{
// 						axis: "h",
// 						origin: segmentStartRef.current as Point,
// 						end: nextOrigin,
// 						dir: goingRight ? 1 : -1,
// 					},
// 				]);
...
// 					x: goingRight ? hDraft.x + hDraft.width - DEFAULT_SIZE : hDraft.x,
// 					y: hDraft.y + DEFAULT_SIZE / 2,
// 				};
...
// 					x: nextOrigin.x,
// 					y: (goingDown ? y + h : y) + DEFAULT_SIZE / 2,
// 				};
// 			}
// 		}
// 	}, [
...
// 					height: seg.height,
// 					edges: {
// 						left: defaultEdgeColor,
// 						right: defaultEdgeColor,
// 						top: defaultEdgeColor,
// 						bottom: defaultEdgeColor,
// 					},
// 					corners: {
// 						"top-left": defaultCornerColor,
// 						"top-right": defaultCornerColor,
// 						"bottom-left": defaultCornerColor,
// 						"bottom-right": defaultCornerColor,
// 					},
// 					clips: {
// 						"top-left": 0,
// 						"top-right": 0,
...
// 		(rectId: string, edge: EdgeName, color: string) => {
// 			setRects((prev) =>
// 				prev.map((r) =>
// 					r.id === rectId ? { ...r, edges: { ...r.edges, [edge]: color } } : r,
// 				),
// 			);
// 			closeMenu();
// 		},
...
// 	const setCornerColor = useCallback(
// 		(rectId: string, corner: CornerName, color: string) => {
// 			setRects((prev) =>
// 				prev.map((r) =>
// 					r.id === rectId
// 						? { ...r, corners: { ...r.corners, [corner]: color } }
// 						: r,
// 				),
// 			);
// 			closeMenu();
...
// 				};
// 			};
// 			const url = await exportJsonToImage({
// 				rects: data.rects ?? [],
// 				images: data.images ?? [],
// 				lines: data.lines ?? [],
...
// 	const handleSpawn1k = useCallback(
// 		() => spawnRandomRects(1000),
// 		[spawnRandomRects],
// 	);
// 	const handleSpawn5k = useCallback(
// 		() => spawnRandomRects(5000),
// 		[spawnRandomRects],
// 	);
...
// 	const getScenePointFromTouchIndex = useCallback(
// 		(evt: TouchEvent, index: number): Point | null => {
...
// 		scenePoint: Point,
// 		groupRects: ReadonlyArray<RectShape>,
...
// 		resizeInitialRectsByIdRef.current = new Map(
// 			groupRects.map((r) => [r.id, r] as const),
// 		);
// 		const memberRectIds = new Set(groupRects.map((r) => r.id));
// 		const gid = groupRects[0]?.groupId ?? null;
// 		const memberImages = images.filter((im) =>
// 			gid
// 				? im.groupId === gid ||
// 					(im.parentRectId && memberRectIds.has(im.parentRectId))
// 				: im.parentRectId
// 					? memberRectIds.has(im.parentRectId)
// 					: false,
// 		);
// 		resizeInitialImagesByIdRef.current = new Map(
// 			memberImages.map((im) => [im.id, im] as const),
// 		);
...
// 		const memberRectIds = new Set(groupRects.map((r) => r.id));
// 		dragAllInitialImagesRef.current = gid
// 			? images.filter(
// 					(im) =>
// 						im.groupId === gid ||
// 						(im.parentRectId && memberRectIds.has(im.parentRectId)),
...
// 			: images;
// 		dragAllInitialRectsByIdRef.current = new Map(
// 			dragAllInitialRectsRef.current.map((r) => [r.id, r] as const),
// 		);
// 		dragAllInitialImagesByIdRef.current = new Map(
// 			dragAllInitialImagesRef.current.map((im) => [im.id, im] as const),
// 		);
// 		isDrawing.current = false;
// 		startPoint.current = null;
// 		direction.current = null;
...
// 	function finalizeResizeDrafts(
// 		drafts: ReadonlyArray<RectDraft>,
// 		maybeNewRect: RectDraft | null,
// 		groupId: string | null,
// 	) {
// 		const toCommit: RectDraft[] = [
// 			...drafts,
// 			...(maybeNewRect ? [maybeNewRect] : []),
// 		];
...
// 		(point: Point): RectShape | null => htFindRect(rects, point),
// 		[rects],
// 	);
...
// 			const centerClient: Point = {
// 				x: (t1.clientX + t2.clientX) / 2,
// 				y: (t1.clientY + t2.clientY) / 2,
// 			};
// 			const centerContainer = getContainerPointFromEvent({
// 				clientX: centerClient.x,
// 				clientY: centerClient.y,
// 			} as unknown as MouseEvent);
...
// 				<Line
// 					points={[r.x, r.y - labelOffset, r.x + r.width, r.y - labelOffset]}
// 					stroke="blue"
...
// 				<Line
// 					points={[r.x - labelOffset, r.y, r.x - labelOffset, r.y + r.height]}
// 					stroke="blue"
// 					strokeWidth={1 / stageScale}
// 					listening={false}
// 				/>
...
// 					<label className="text-gray-700 text-sm" htmlFor="vain-bg-input">
// 						Match Image
// 					</label>
// 					<input
// 						id="vain-bg-input"
...
// 					<label className="text-gray-700 text-sm" htmlFor="vain-group">
// 						Group
// 					</label>
// 					<select
// 						id="vain-group"
...
// 					</select>
// 					{/* Scale */}
// 					<label className="text-gray-700 text-sm" htmlFor="vain-scale">
// 						Scale
// 					</label>
// 					<input
// 						id="vain-scale"
...
// 						value={
// 							(vainActiveGroupKey &&
// 								vainGroupTransforms.get(vainActiveGroupKey)?.scale) ??
// 							1
// 						}
// 						onChange={(e) => {
...
// 					/>
// 					{/* Rotation */}
// 					<label className="text-gray-700 text-sm" htmlFor="vain-rotation">
// 						Rotation
// 					</label>
// 					<input
// 						id="vain-rotation"
...
// 						value={
// 							(vainActiveGroupKey &&
// 								vainGroupTransforms.get(vainActiveGroupKey)?.rotation) ??
// 							0
// 						}
// 						onChange={(e) => {
// 							const v = Number.parseFloat(e.target.value);
...
// 						distanceClick={(rectId, sinkId, value) =>
// 							setEditDistance({ isOpen: true, rectId, sinkId, value })
// 						}
// 						images={images}
// 					/>
// 					<DragOverlayLayer
// 						rects={rects}
// 						active={sinkDrag.active}
// 						candidateRectId={sinkDrag.candidateRectId}
// 					/>
...
// 								{Array.from(collectGroups(rects).entries()).map(
// 									([key, gRects]) => {
// 										const t = vainGroupTransforms.get(key) ?? {
// 											x: 0,
// 											y: 0,
// 											scale: 1,
// 											rotation: 0,
...
// 								<Line
// 									points={
// 										seamPreview.orientation === "v"
// 											? [
// 													seamPreview.at,
// 													seamPreview.bounds.top,
// 													seamPreview.at,
// 													seamPreview.bounds.bottom,
// 												]
// 											: [
// 													seamPreview.bounds.left,
// 													seamPreview.at,
// 													seamPreview.bounds.right,
// 													seamPreview.at,
// 												]
// 									}
// 									stroke="#ef4444"
...
// 				images={images}
// 				lines={lines}
// 				onOpenDistance={(rectId, imageId, value) =>
// 					setEditDistance({ isOpen: true, rectId, sinkId: imageId, value })
// 				}
// 			/>
// 			<DistanceModal
...
// 					setImages((prev) =>
// 						prev.map((img) =>
// 							img.id === im.id ? { ...img, x: clampedX } : img,
// 						),
// 					);
// 					setEditDistance({
// 						isOpen: false,
...
// 				onClose={closeMenu}
// 				onSetEdgeColor={(rectId, edge, color) =>
// 					setEdgeColor(rectId, edge, color)
// 				}
// 				onSetCornerColor={(rectId, corner, color) =>
// 					setCornerColor(rectId, corner, color)
// 				}
...
// 					setClipModal((s) => ({
// 						...s,
// 						value: Math.max(0, Math.min(n, s.max)),
// 					}))
// 				}
// 				onCancel={() =>
// 					setClipModal({
...
// 				}
// 				onApply={() => {
// 					if (!clipModal.isOpen || !clipModal.rectId || !clipModal.corner)
// 						return;
// 					const cornerKey = clipModal.corner as CornerName;
// 					setRects((prev) =>
// 						prev.map((r) =>
// 							r.id === clipModal.rectId
// 								? {
// 										...r,
// 										clips: {
// 											...(r.clips ?? {
...
// 											[cornerKey]: Math.max(
// 												0,
// 												Math.min(clipModal.value, clipModal.max),
// 											),
// 										},
// 									}
// 								: r,
...
// 				}
// 				onApply={() => {
// 					if (!radiusModal.isOpen || !radiusModal.rectId || !radiusModal.corner)
// 						return;
// 					const cornerKey = radiusModal.corner as CornerName;
// 					setRects((prev) =>
// 						prev.map((r) =>
// 							r.id === radiusModal.rectId
// 								? {
// 										...r,
...
// 											[cornerKey]: Math.max(
// 												0,
// 												Math.min(radiusModal.value, radiusModal.max),
// 											),
// 										},
// 									}
// 								: r,
