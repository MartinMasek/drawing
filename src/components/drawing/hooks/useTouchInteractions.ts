...
import type { Point, RectDraft, DragDirection } from "../types";
...
type Deps = {
  ...
  draftForDrag: (origin: Point, dx: number, dy: number, dir: DragDirection) => RectDraft;
...
export function useTouchInteractions({ tool, toScene, getContainerPointFromEvent, getScenePointFromTouchIndex, draftForDrag, setNewRect, isDrawingRef, startPointRef, directionRef, openAtContainerPoint }: Deps) {
  ...
  const onTouchStart = useCallback((e: { evt: TouchEvent; target?: unknown }) => {
    if (e.evt.touches.length === 1) {
      ...
      if (tool === "rect") {
        touchLongPressTimer.current = window.setTimeout(() => {
          const start = getScenePointFromTouchIndex(e.evt, 0) ?? toScene(containerPoint);
          touchHasLongPressed.current = true;
          isDrawingRef.current = true;
          startPointRef.current = start;
          directionRef.current = null;
          setNewRect({ x: start.x, y: start.y - 100 / 2, width: 100, height: 100 });
        }, LONG_PRESS_MS);
      }
      return;
    }
    clearTouchTimers();
  }, [clearTouchTimers, directionRef, getContainerPointFromEvent, getScenePointFromTouchIndex, isDrawingRef, setNewRect, startPointRef, toScene, tool]);

  const onTouchMove = useCallback((e: { evt: TouchEvent }) => {
    if (e.evt.touches.length === 1) {
      const t = e.evt.touches.item(0);
      if (!t) return;
      const containerPoint = getContainerPointFromEvent({ clientX: t.clientX, clientY: t.clientY } as unknown as MouseEvent);
      ...
      if (tool === "rect") {
        ...
        const draft = draftForDrag(startPointRef.current, dx, dy, directionRef.current);
        setNewRect(draft);
        return;
      }
      return;
    }
  }, [clearTouchTimers, directionRef, draftForDrag, getContainerPointFromEvent, getScenePointFromTouchIndex, isDrawingRef, setNewRect, startPointRef, tool]);

  const onTouchEnd = useCallback((e: { evt: TouchEvent; target?: { getStage?: () => { container: () => { getBoundingClientRect: () => DOMRect } } } }) => {
    if (e.evt.touches.length === 0) {
      clearTouchTimers();
      if (tool !== "rect") {
        if (!touchMovedBeyondTolerance.current && touchStartContainerPoint.current) {
