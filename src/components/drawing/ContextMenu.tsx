import { Popover, PopoverButton, PopoverCloseButton, PopoverPanel } from "../Popover";
...
type Props = {
  ...
  onClose: () => void;
  onSetEdgeColor: (rectId: string, edge: "left" | "right" | "top" | "bottom", color: string) => void;
  onSetCornerColor: (rectId: string, corner: "top-left" | "top-right" | "bottom-left" | "bottom-right", color: string) => void;
  onAction: (action: string) => void;
};

export default function ContextMenu({ menu, mode, popoverButtonRef, onClose, onSetEdgeColor, onSetCornerColor, onAction }: Props) {
  if (!menu.isOpen) return null;
  return (
    <div
      style={{ position: "fixed", left: menu.clientX, top: menu.clientY, zIndex: 1000 }}
    >
      <Popover>
        ...
        <PopoverPanel className="min-w-[180px] rounded-md border border-gray-200 bg-white p-1 shadow-lg">
          ...
          {mode !== "sink" && menu.target?.kind === "edge" && (
            <div className="mb-1 px-3 py-1 text-xs text-gray-500">Edge color</div>
          )}
          {mode !== "sink" && menu.target?.kind === "edge" && (
            <div className="mb-1 grid grid-cols-6 gap-1 px-2">
              {EDGE_COLOR_OPTIONS.map((c) => (
                <button
                  ...
                  type="button"
                  onClick={() => {
                    if (!menu.rectId || !menu.target || menu.target.kind !== "edge") return;
          ...
          {mode !== "sink" && menu.target?.kind === "corner" && (
            <div className="mb-1 px-3 py-1 text-xs text-gray-500">Corner color</div>
          )}
          {mode !== "sink" && menu.target?.kind === "corner" && (
            <div className="mb-1 grid grid-cols-6 gap-1 px-2">
              {EDGE_COLOR_OPTIONS.map((c) => (
                <button
                  ...
                  type="button"
                  onClick={() => {
                    if (!menu.rectId || !menu.target || menu.target.kind !== "corner") return;
