...
export default function CornerRadiusModal({ isOpen, cornerLabel, value, maxValue, onChange, onCancel, onApply }: Props) {
  ...
  return (
    <button
      ...
      onKeyUp={(e) => { if (e.key === "Escape" || e.key === "Enter") onCancel(); }}
    >
      <dialog open className="w-full max-w-sm rounded-md border border-gray-200 bg-white p-4 shadow-lg" onClick={(e) => e.stopPropagation()}>
        <div className="mb-3 text-base font-semibold text-gray-900">Round corner: {cornerLabel}</div>
        <div className="mb-2 text-sm text-gray-700">Radius (px) — 0 to {Math.floor(maxValue)}</div>
        <input
          type="number"
          min={0}
