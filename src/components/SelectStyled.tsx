...
>(
    ...
    const [menuOpen, setMenuOpen] = React.useState<boolean | undefined>(undefined)

    const onOptionChange = (selectedOptions: OnChangeValue<Option, IsMulti>, actionMeta: ActionMeta<Option>) => {
        reactSelectProps.onChange?.(selectedOptions, actionMeta)

        if (
            reactSelectProps.isMulti &&
            reactSelectProps.options?.length === (selectedOptions as OnChangeValue<Option, true>).length
        ) {
            setMenuOpen(false)
        }
    }
    ...
    const inputId = reactSelectProps.inputId ?? reactSelectProps.id ?? reactSelectProps.name
    ...
    const defaultClassNames = {
        control: (state: ControlProps<Option, IsMulti, Group>) =>
            clsx(
                ...
                state.isDisabled && '!text-text-neutral-disabled !border-border-input-disabled !bg-background-input-disabled',
                error ? 'border-border-input-error' : 'border-border-input-default hover:border-border-input-hover',
                inputSize !== 'sm' && 'px-[4px] py-[3px]'
            ),
        input: () => clsx('!text-text-neutral-primary', inputSize === 'sm' && 'text-sm'),
        // biome-ignore lint/suspicious/noExplicitAny: <explanation>
        singleValue: (state: any) =>
            clsx(
                inputSize === 'sm' && 'h-[20px] text-sm',
                state.isDisabled ? '!text-text-input-disabled bg-red' : '!text-text-neutral-primary'
            ),
        indicatorSeparator: () => 'hidden',
        menuList: () => clsx('!bg-background-neutral-primary-default', inputSize === 'sm' && 'text-sm'),
        menuPortal: () => '!z-[100]',
              ...
              ...Object.fromEntries(
                  Object.entries(customClassNames).map(([key, value]) => [
                      ...
                      typeof value === 'function'
                          ? (state: Parameters<NonNullable<(typeof defaultClassNames)[keyof typeof defaultClassNames]>>[0]) =>
                                clsx(defaultClassNames[key as keyof typeof defaultClassNames]?.(state), value(state))
                          : value,
                  ])
              ),
          }
...
>(props: OptionProps<Option, IsMulti, Group>) {
    ...
    const content = (hasChildren ? props.children : (props.label as React.ReactNode)) as React.ReactNode
    ...
    return (
        ...
        >
            <div className={clsx('flex-1', !isElement && 'text-text-neutral-secondary')}>{content}</div>
            {props.isSelected && (
