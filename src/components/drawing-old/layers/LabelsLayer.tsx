...
import type { LineShape, Point, RectDraft, RectShape } from "../types";
			...
			<Layer listening={false} hitGraphEnabled={false}>
				{lines.map((ln) => (
					<Line
						...
						points={
							ln.kind === "v"
								? [ln.at, -100000, ln.at, 100000]
								: [-100000, ln.at, 100000, ln.at]
						}
						stroke="#94a3b8"
						dash={[6, 4]}
						strokeWidth={1}
				...
				{(rects.length <= 300 || stageScale > 1.5) && (
					<>
						...
						{draftSegments?.map((seg) => (
							<Fragment
								key={`label-draft-${seg.x}-${seg.y}-${seg.width}-${seg.height}`}
							>
								{renderLabels(seg)}
							</Fragment>
						))}
						...
						return sinks.map((img, idx) => {
							...
							return (
								<Text
									...
									listening
									onClick={() =>
										distanceClick(r.id, img.id, Math.max(0, centerX - r.x))
									}
								/>
							);
						});
