...
export async function exportJsonToImage(json: CanvasJson): Promise<string> {
	...
	for (const r of json.rects ?? []) {
		...
		rectLayer.add(
			new Konva.Line({
				points: [r.x, r.y, r.x + r.width, r.y],
				stroke: r.edges.top,
				listening: false,
			}),
		);
		...
		rectLayer.add(
			new Konva.Line({
				points: [r.x + r.width, r.y, r.x + r.width, r.y + r.height],
				stroke: r.edges.right,
				listening: false,
			}),
		);
		// Bottom
		rectLayer.add(
			new Konva.Line({
				points: [r.x + r.width, r.y + r.height, r.x, r.y + r.height],
				stroke: r.edges.bottom,
				listening: false,
			}),
		);
		// Left
		rectLayer.add(
			new Konva.Line({
				points: [r.x, r.y + r.height, r.x, r.y],
				stroke: r.edges.left,
				listening: false,
			}),
		);
	...
	for (const ln of json.lines ?? []) {
		if (ln.kind === "v") {
			linesLayer.add(
				new Konva.Line({
					points: [ln.at, -100000, ln.at, 100000],
					stroke: "#94a3b8",
					dash: [6, 4],
					listening: false,
				}),
			...
			linesLayer.add(
				new Konva.Line({
					points: [-100000, ln.at, 100000, ln.at],
					stroke: "#94a3b8",
					dash: [6, 4],
					listening: false,
				}),
