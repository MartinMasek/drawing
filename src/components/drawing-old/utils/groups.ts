...
export function getGroupRectsForRect(
	allRects: ReadonlyArray<RectShape>,
	rect: RectShape,
...
export function collectGroups(
...
): Map<string, ReadonlyArray<RectShape>> {
	...
	return new Map(
		Array.from(byGroup.entries()).map(([k, v]) => [
			k,
			v as ReadonlyArray<RectShape>,
		]),
	);
}
