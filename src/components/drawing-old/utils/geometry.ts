...
export function getEffectiveCornerOffsets(
	rect: RectShape,
): Record<CornerKey, number> {
	const clips = rect.clips ?? {
		"top-left": 0,
		"top-right": 0,
		"bottom-left": 0,
		"bottom-right": 0,
	};
	const radii = rect.radii ?? {
		"top-left": 0,
		"top-right": 0,
		"bottom-left": 0,
		"bottom-right": 0,
 ...
 */
export function addRectPathWithCorners(
	...
	const rtl = radii["top-left"];
	const rtr = radii["top-right"];
	const rbl = radii["bottom-left"];
	const rbr = radii["bottom-right"];
	const ctl = clips["top-left"];
	const ctr = clips["top-right"];
	const cbl = clips["bottom-left"];
	const cbr = clips["bottom-right"];
	...
	// Top-right corner
	if (arcTR) {
		ctx.arc(x + w - rtr, y + rtr, rtr, -Math.PI / 2, 0);
	} else if (tr > 0) {
		ctx.lineTo(x + w, y + tr);
	}
	...
	// Bottom-right corner
	if (arcBR) {
		ctx.arc(x + w - rbr, y + h - rbr, rbr, 0, Math.PI / 2);
	} else if (br > 0) {
	...
	// Bottom-left corner
	if (arcBL) {
		ctx.arc(x + rbl, y + h - rbl, rbl, Math.PI / 2, Math.PI);
	} else if (bl > 0) {
		ctx.lineTo(x, y + h - bl);
	}
	...
	// Top-left corner
	if (arcTL) {
		ctx.arc(x + rtl, y + rtl, rtl, Math.PI, 1.5 * Math.PI);
	} else if (tl > 0) {
		ctx.lineTo(x + tl, y);
	}
