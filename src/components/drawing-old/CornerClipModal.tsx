...
export default function CornerClipModal({ isOpen, cornerLabel, value, maxValue, onChange, onCancel, onApply }: Props) {
  ...
  return (
    ...
    >
      <dialog open className="w-full max-w-sm rounded-md border border-gray-200 bg-white p-4 shadow-lg" onClick={(e) => e.stopPropagation()}>
        <div className="mb-3 text-base font-semibold text-gray-900">Clip corner: {cornerLabel}</div>
        <div className="mb-2 text-sm text-gray-700">Distance (px) — 0 to {Math.floor(maxValue)}</div>
        <input
          type="number"
          min={0}
