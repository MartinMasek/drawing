...
export function Divider({ orientation = 'horizontal', text, className }: DividerProps) {
    if (text && orientation === 'horizontal') {
        return (
            <div className={cn('flex w-full items-center gap-2', className)}>
                <div className='h-px flex-1 bg-border-neutral' />
                <span className='whitespace-nowrap text-sm text-text-neutral-secondary'>{text}</span>
                <div className='h-px flex-1 bg-border-neutral' />
            </div>
        )
    }
