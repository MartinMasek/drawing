...
const buttonVariants = cva('inline-flex items-center gap-2 rounded-lg font-semibold transition-all md:whitespace-nowrap', {
    variants: {
        size: {
    ...
    compoundVariants: [
        ...
        },
        {
            variant: 'outlined',
            color: 'always-black',
            className: 'border border-black text-black hover:bg-black/20 hover:text-black',
        },
        { variant: 'link', color: 'always-black', className: 'text-black hover:text-gray-700' },
        {
            variant: 'outlined',
            color: 'always-white',
            className: 'border border-white text-white hover:bg-white/20 hover:text-white',
        },
        { variant: 'link', color: 'always-white', className: 'text-white hover:text-gray-200' },
    ],
    defaultVariants: {
        size: 'md',
        variant: 'contained',
...
const Button = React.forwardRef<HTMLButtonElement, React.PropsWithChildren<ButtonProps>>(
    (
        ...
        return (
                <button
                    {...buttonProps}
                    className={cn('cursor-pointer',buttonVariants({ size, variant, color, iconOnly, loading, disabled }), className)}
