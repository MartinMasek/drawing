...
export interface IconProps extends VariantProps<typeof iconVariants>, React.PropsWithChildren {
    className?: string
}

export function Icon({ size, color, className, children }: IconProps) {
    return <span className={iconVariants({ className, size, color })}>{children}</span>
}
